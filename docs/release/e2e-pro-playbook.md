...
> Cirujano adoption (2026-09-08): Wave A is adopted as the release gate. Waves B to H are
> not adopted yet; the project has no release. Re-evaluate by risk before the first tag.
> The Project Adaptation Profile below carries verified values; no release-report command exists yet.
...
| Primary product type | `MONOREPO` (GitHub Action + CLI) |
| Package/build system | `pnpm 11 workspaces, tsc + esbuild` |
| Integration branch | `develop` |
| Production branch | `main` |
| Merge strategy | `SQUASH` (only method enabled; delete branch on merge) |
| Release artifact | `PACKAGE` (npm `cirujano`) and committed Action bundle `packages/action/dist/index.cjs`, tagged from `main` |
| Deployment provider | none (GitHub Releases + npm) |
| Local test target | developer machine, `pnpm run ci:fast` |
| Preview target | none |
| Staging target | none |
| Production target | GitHub Marketplace Action + npm package consumers |
| Test runner(s) | `vitest` |
| Unit command | `pnpm run test` |
| Integration command | none yet (planned: replay of captured Actions API payloads) |
| E2E command | none yet (planned: audit and patch a public fixture repository) |
| Typecheck command | `pnpm run typecheck` |
| Lint command | `pnpm run lint` |
| Build command | `pnpm run build` then `node scripts/verify-bundle.mjs` |
| Release-report command | none yet (Wave A deliverable) |
| Primary datastore | none |
| Object/media storage | none |
| Queue/event system | GitHub Actions events (`workflow_job` queued) |
| Authentication | GitHub token (Actions read, PR write); Nebius service account |
| Payments/entitlements | none |
| Email/notifications | none |
| Other external vendors | Nebius Token Factory (Nemotron inference), Nebius AI Cloud (runner VMs), GitHub Actions API |
| Observability | GitHub run logs and Action outputs; no error tracking yet |
| Hardware/real-device surfaces | none |
| Agent command directory | `.claude/commands/` |
| Capability registry owner | repository owner |
| Release approver | repository owner |
| Rollback authority | repository owner |
